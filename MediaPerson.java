...
public class MediaPerson implements Serializable{
	...
	public void setActingStartIndex(int index){
		actingStartIndex = index;
	}
	
	public void setDirectingStartIndex(int index){
		directingStartIndex = index;
	}
	...
	public String getProfession(){
		...
		if(moviesActed != 0 || seriesActed != 0)
			profs += "Actor ";
		
		if(moviesDirected != 0 || seriesDirected != 0)
			profs += "Director ";
			
		if(moviesProduced != 0 || seriesProduced != 0)
			profs += "Producer";
	...
	ArrayList<Integer> getCredits(Integer year){
		...
		returnList.add(year);
		for(Media w: works){
			if(w.getYear().contains(year.toString() )){
