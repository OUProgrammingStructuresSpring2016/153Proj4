		...
		while (currentLine != null) {
			...
			if (currentLine.contains("-?") || currentLine.contains("-1")	|| currentLine.contains("-2")) {
				...
				if (epData.contains("{{")) { // if episode was suspended
					...
					String seasonNums = epData.substring(epData.lastIndexOf("(") + 1, epData.lastIndexOf(")"));

					newSeries.addEpisode(new TVEpisode(epTitle, seasonNums, epYear, newSeries.getTitle()));

				} else { // episode has no season/episode number

					String epTitle = epData.substring(1, epData.lastIndexOf("}"));
